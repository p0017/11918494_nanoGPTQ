...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attention = nn.Linear(config.embedding_dimension, 3 * config.embedding_dimension, bias=config.bias)
        # linear layer for combining the output of the different attention heads
        self.c_projection = nn.Linear(config.embedding_dimension, config.embedding_dimension, bias=config.bias)
        # regularization
        ...
        if not self.flash_attention:
            ...
            causal_mask = causal_mask.view(1, 1, config.context_length, config.context_length)
    ...
    def forward(self, x):
        ...
        qkv = self.c_attention(x)  # (batch_size, sequence_length, 3 * embedding_dim)
        query, key, value = qkv.split(self.embedding_dim, dim=2) # each is (batch_size, sequence_length, embedding_dim)

        # Reshape q, k, v for multi-head attention
        # We split the embedding dimension into multiple heads
        key = key.view(batch_size, sequence_length, self.n_attention_heads, embedding_dim // self.n_attention_heads)
        query = query.view(batch_size, sequence_length, self.n_attention_heads, embedding_dim // self.n_attention_heads)
        value = value.view(batch_size, sequence_length, self.n_attention_heads, embedding_dim // self.n_attention_heads)
        ...
        if self.flash_attention:
            if self.training:
                y = torch.nn.functional.scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=self.dropout, is_causal=True)
            else:
                y = torch.nn.functional.scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=True)
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.first_linear = nn.Linear(config.embedding_dimension, 4 * config.embedding_dimension, bias=config.bias)
        self.gelu = nn.GELU()
        # Second linear layer reduces the dimension back to the original embedding size
        self.second_linear = nn.Linear(4 * config.embedding_dimension, config.embedding_dimension, bias=config.bias)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            token_embedding = nn.Embedding(config.vocabulary_size, config.embedding_dimension),
            # Adds an unique vector for every position in the input sequence
            positional_embedding = nn.Embedding(config.context_length, config.embedding_dimension),
            dropout = nn.Dropout(config.dropout),
            # Stack of transformer blocks
            transformer_blocks = nn.ModuleList([TransformerBlock(config) for _ in range(config.n_layers)]),
            # Final layer norm before output
            final_layer_norm = LayerNorm(config.embedding_dimension, bias=config.bias)
        ))
        # Linear layer for mapping the final embeddings to logits over the vocabulary
        self.linear_mapping_head = nn.Linear(config.embedding_dimension, config.vocabulary_size, bias=False)
    ...
    def forward(self, input_token_indices, targets=None):
        ...
        assert sequence_length <= self.config.context_length, "Cannot forward, sequence length exceeds context length"

        # Creating a position index tensor [0, 1, 2, ..., sequence_length-1] for positional embeddings
        positional_indices = torch.arange(0, sequence_length, dtype=torch.long, device=device)

        token_embeddings = self.transformer.token_embedding(input_token_indices)  # (batch_size, sequence_length, embedding_dimension)
        positional_embeddings = self.transformer.positional_embedding(positional_indices)  # (sequence_length, embedding_dimension)
        ...
        if targets is not None: # Training mode
            ...
            logits = self.linear_mapping_head(x)  # (batch_size, sequence_length, vocabulary_size)
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)
        else: # Inference mode
            logits = self.linear_mapping_head(x[:, [-1], :]) # Only return logits for the last token
            loss = None
    ...
    def crop_context_length(self, context_length: int):
        ...
        assert context_length <= self.config.context_length, "New context length must be less than or equal to the original context length."
        self.config.context_length = context_length
        self.transformer.positional_embedding = nn.Parameter(self.transformer.positional_embedding.weight[:context_length])
        for block in self.transformer.transformer_blocks:
    ...
    def configure_optimizers(self, weight_decay:float, learning_rate:float, betas:tuple, device_type:str):
        ...
        no_weight_decay_parameters = [p for n, p in parameter_dict.items() if p.ndim < 2]
        ...
        fused_adamw_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters and device_type == 'cuda'
        extra_arguments = dict(fused=True) if fused_adamw_available else dict()
        optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=learning_rate, betas=betas, **extra_arguments)
        print(f"Using {'fused ' if fused_adamw_available else ''}AdamW optimizer")
    ...
    @beartype
    @torch.no_grad()
    def generate(self, input_token_indices, max_new_tokens:int, temperature:float=1.0, top_k:int=None):
        '''
        ...
        for _ in range(max_new_tokens):
            ...
            # Otherwise, crop the input
                ...
                input_token_indices_condition = input_token_indices[:, -self.config.context_length:]
            ...
            input_token_indices = torch.cat((input_token_indices, next_token_index), dim=1)
